...
export default function Products() {
 ...
 <p className="text-sm text-white mt-1">{products.length} products · {products.filter(p => p.isActive).length} active</p>
 </div>
 ...
 product.isActive ? 'text-emerald-400 bg-emerald-400/[0.1] ' : 'text-white bg-k-border/50 '
 }`}>
 ...
 <p className="text-xs text-white mb-3 line-clamp-2">{product.description || 'No description'}</p>
 <div className="flex items-center justify-between mb-4">
 <span className="text-xs text-white px-2 py-0.5 rounded bg-k-border/50">{product.material || 'N/A'}</span>
 ...
 product.isActive ? 'text-emerald-400 hover:text-emerald-300' : 'text-white hover:text-white'
 }`}
 >
 ...
 <button onClick={() => openEdit(product)} className="w-7 h-7 rounded-lg flex items-center justify-center text-white hover:text-white hover:bg-white/[0.06] transition-all">
 <Edit3 size={13} />
 </button>
 <button onClick={() => handleDelete(product.id)} className="w-7 h-7 rounded-lg flex items-center justify-center text-white hover:text-red-400 hover:bg-red-400/[0.06] transition-all">
 <Trash2 size={13} />
 </button>
 </div>
 </div>
 ...
 {products.length === 0 && (
 <div className="col-span-full py-12 text-center text-white text-sm border border-dashed border-k-border rounded-xl">
 ...
 <button onClick={() => setModalOpen(false)} className="px-5 py-2.5 text-sm text-white border border-white/10 rounded-xl hover:text-white hover:border-k-silver/40 transition-all">Cancel</button>
 <button 
 onClick={handleSave} 
